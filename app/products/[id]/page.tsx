        ...
        <div className="bg-white border-b border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <nav className="flex items-center text-xs font-semibold text-slate-500 gap-2">
              ...
              <ChevronRight className="w-3.5 h-3.5" />
              <span className="text-secondary truncate max-w-[200px] sm:max-w-xs block">
                {product.name}
              </span>
            </nav>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-16">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {[...Array(4)].map((_, i) => (
                    <div
                      key={i}
                      className={`aspect-square rounded-xl border ${i === 0 ? "border-primary ring-2 ring-primary/20 bg-primary/5" : "border-border bg-muted"} flex items-center justify-center opacity-80 hover:opacity-100 transition-opacity cursor-pointer relative overflow-hidden`}
                    >
                      {product.imageUrl && i === 0 ? (
                        <Image
                          ...
                          alt="Thumb"
                          fill
                          className="object-cover scale-150 p-2 opacity-50"
                        />
                      ) : (
                        <Box
                          className={`w-5 h-5 ${i === 0 ? "text-primary" : "text-slate-300"}`}
                        />
                      )}
                    </div>
                  ))}
              ...
              <div className="flex flex-col">
                <div className="mb-8">
                  ...
                  <h1 className="text-3xl sm:text-4xl font-bold text-heading leading-[1.15] mb-4">
                    {product.name}
                  </h1>
                  ...
                  <div className="border border-border rounded-xl overflow-hidden shadow-sm">
                    ...
                    <div className="bg-white">
                      <div className="grid grid-cols-2 divide-x divide-border border-b border-border text-sm">
                        <div className="px-6 py-4 font-semibold text-slate-600">
                          HSN
                        </div>
                        <div className="px-6 py-4 text-heading font-medium font-mono">
                          {product.sku}
                        </div>
                      </div>
                      <div className="grid grid-cols-2 divide-x divide-border border-b border-border text-sm">
                        ...
                        <div className="px-6 py-4 text-heading font-medium">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                {relatedProducts.map((rel) => (
                  <Link
                    ...
                    key={rel.id}
                    className="group flex flex-col bg-white rounded-2xl border border-border overflow-hidden hover:border-primary/30 hover:shadow-lg transition-all duration-300"
                  >
                    ...
                    <div className="p-4 flex flex-col justify-between flex-grow">
                      <h3 className="font-bold text-heading text-sm mb-4 leading-snug group-hover:text-primary transition-colors">
                        {rel.name}
                      </h3>
